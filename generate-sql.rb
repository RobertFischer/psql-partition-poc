...
File.open("#{dir}/001_partition.sql", "w") do |file|
	age_range.each do |age|
		...
		file.puts("CREATE TABLE tbl_#{birthyear}_birthday ( CHECK ( #{birthyear} = EXTRACT( year FROM birthdate ) ) ) INHERITS ( tbl_main );")
	end
end
	...
	age_range.each do |age|
		...
		file.puts(<<HEREDIR
				IF ( EXTRACT(year FROM NEW.birthdate) = #{birthyear} ) THEN
					INSERT INTO tbl_#{birthyear}_birthday VALUES (NEW.*);
					RETURN NULL;
				END IF;
HEREDIR
		)
